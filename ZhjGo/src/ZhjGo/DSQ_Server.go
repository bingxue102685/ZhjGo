...
func initDSQNetRPC() {
	...
	ConnDSQRPC = client
	cacheDSQ = cache2go.Cache("ZhjGo_DSQ")
}
...
func initConnDSQ(conn *websocket.Conn) {
	...
	data := &Proto2.DSQ2GW_ConnServer{
		...
		ServerID:  util.MD5_ZhjGo("8895" + "DSQ server"),
	}
	fmt.Println(data)
...
func GW2DSQ_PlayerGiveUpProto2Fucn(conn *websocket.Conn, ProtocolData map[string]interface{}) {
	...
	//--------------------------------------------------------------------------
	return
	// gamelev := util.Sort_ZhjGo(conf.DSQGameExp, expdata+data.GameExp)
}
...
func DSQ2GW_PlayerGameInitProto2Fucn(conn *websocket.Conn, ProtocolData map[string]interface{}) {
	...
	iRoomID := util.Str2int_ZhjGo(StrRoomID)
...
func CacheUpdateRoomData(iRoomID int, Update_pos string, value int) {
	...
	for i := 0; i < len(strsplit); i++ {
		if i == 0 {
			ipos_x = util.Str2int_ZhjGo(strsplit[i])
		} else {
			ipos_y = util.Str2int_ZhjGo(strsplit[i])
		}
	}
...
func CacheMoveChessIsUpdateData(iRoomID int, Update_pos string, MoveDir int, stropenid string) (string, string, string) {
	...
	strnewpos := util.Int2str_ZhjGo(ipos_x) + "," + util.Int2str_ZhjGo(ipos_y)
	if ihoulai == 0 {
		...
		res.Data().(*RoomPlayerDSQ).LeftTime = int(util.GetNowUnix_ZhjGo())
...
func InitDSQ(data1 []int) [4][4]int {
	...
	for i := 0; i < Proto2.Mouse*2; i++ {
		icount := util.RandInterval_ZhjGo(0, int32(len(data))-1)
