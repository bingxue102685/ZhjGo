...
// 链接 http://127.0.0.1:8892/ZhjGoGM?Protocol=11&Protocol2=1&UID=&Itype=&ModifyNum=
				...
				case strconv.Itoa(Proto2.W2GMS_Update_PlayerEmailDataProto2):
					{
						...
						if bOPType { // 1 新增 2 编辑 3 删除
							if OPType[0] == "1" {
								...
								stb := &player.EmailGM{}
								json.Unmarshal([]byte(EmailData[0]), &stb)
								EmailDatatmp1[util.Str2int_ZhjGo(UID[0])] = stb
							} else if OPType[0] == "3" {
								UID, _ := req.Form["UID"]
								delete(EmailDatatmp1, util.Str2int_ZhjGo(UID[0]))
							}
